            ...
            if(args[1] != undefined) {
                if(String(args[0]).toLowerCase() === 'everyone') {
                    if (client.userdata.get(authorID, "images") === true) { 
                        return message.channel.send(eie.embedImageEveryone('cry', client, message, args));
                    } else {
                        return message.channel.send(eine.embedNoImageEveryone('cry', client, message, args));
                    }
                }
            }
            if (client.userdata.get(authorID, "images") === true) { //No mention, WITH images
                return message.channel.send(eis.embedImageSelf('cry', client, message, args));
            } else { //No mention, WITHOUT images
                return message.channel.send(eins.embedNoImageSelf('cry', client, message, args));
            }
            default: //If a member is mentioned
                if (client.userdata.get(authorID, "images") === true) { //Mention, WITH images
                    return message.channel.send(ei.embedImage('cry', client, message, args));
                } else { //Mention, WITHOUT images
                return message.channel.send(ein.embedNoImage('cry', client, message, args));
                }
    }
}